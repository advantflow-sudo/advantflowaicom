...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch project details using service role to bypass RLS
    ...
    if (remaining <= 0) throw new Error("No outstanding balance");
      ...
      line_items: [
        {
          price_data: {
            ...
            unit_amount: Math.round(remaining * 100), // Convert to pence
          },
          quantity: 1,
        },
      ],
      mode: "payment",
      success_url: `${req.headers.get("origin")}/portal?payment=success&project=${projectId}`,
