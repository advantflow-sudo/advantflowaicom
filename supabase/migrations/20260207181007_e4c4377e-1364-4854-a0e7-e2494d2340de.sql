...
CREATE POLICY "Clients can view own projects" ON public.client_projects FOR SELECT USING (auth.uid() = client_id);
CREATE POLICY "Admins can manage all projects" ON public.client_projects FOR ALL USING (public.has_role(auth.uid(), 'admin'));
